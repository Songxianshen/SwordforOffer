...
从头到尾一次扫描这个数组中的每个数字。当扫描到下标为*i*的数字时，我们将这个数字用m来表示，首先比较m是不是等于i。如果是，则接着扫描下一个数字；如果不是，则拿它和第*m*个数字进行比较。如果它和第*m*个数字相等；就找到了一个重复的数字（该数字在下标为*i*和*m*的位置都出现了）；如果它和第*m*个不相等，就把第*i*个数字和第*m*个数字交换，**把*m*放到属于它的位置**。接下来再重复比较、交换的过程，直到我们发现一个重复的数字。
